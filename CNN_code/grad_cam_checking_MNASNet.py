...
model_weights_path = "/home/joey/CIDAUT/model_output/run21/MNASNet.pth"
...
def generate_and_display_heatmaps(model, dataloader, target_layer, device, output_folder, num_images=3):
    ...
    for inputs, labels in dataloader:
        ...
        for i in range(inputs.size(0)):
            ...
            # fake_path = os.path.join(output_folder, f"fake_heatmap_{processed_images}_label_{label}_pred_{pred_label}.png")
            ...
            real_path = os.path.join(output_folder, f"real_heatmap_{processed_images}_label_{label}_pred_{pred_label}.png")
            plt.imsave(real_path, heatmap_real_overlay)
            print(f"Saved real heatmap to: {real_path}")       

            processed_images += 1

target_layer = model.layers[-1]  # target layer for MNASNet
# output_folder = "CNN_code/MNASNet/fake_heatmaps"
output_folder = "CNN_code/MNASNet/real_heatmaps"
